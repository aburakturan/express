...
const Trader = require('./trader/functions');
const Ticker = require('./trader/ticker');
const VolumeTracker = require('./trader/volumeTracker');
...
(async () => {
    ...
    await new Promise((resolve, reject) =>
        require('http')
            .Server(app)
            .listen(Number(process.env.LOCAL_PORT) || 3001, resolve)
            .on('error', reject)
    );
    winston.info('server is running...', process.env.LOCAL_PORT);
})();
